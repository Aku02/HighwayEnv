...
class RacetrackEnv(AbstractEnv):
    ...
    def _agent_reward(self, action: np.ndarray, vehicle: Vehicle) -> float:
        ...
        raw_reward = (
            0.5 * rewards["speed_reward"] +
            0.15 * rewards["lane_centering_reward"] +
            0.05 * rewards["on_road_reward"] +
    ...
    def _agent_rewards(self, action: np.ndarray, vehicle: Vehicle) -> dict[str, float]:
        ...
        upper_bound = 30.0    # Upper bound on the speed for control [m/s]
        ...
        lane_centering_cost = self.config.get("lane_centering_cost", 0.2)
        ...
        reverse_penalty = -((10 - vehicle.speed) / 10 * 0.5) if vehicle.speed < 10 else 0.0
